...
> **注意**：這份文件是 vista.tw **既存設計系統的文件化**，不是新提案。
> 系統的權威來源是 `src/styles/global.css` 的 `:root` design tokens。這份 DESIGN.md
> 是人類可讀的解釋與決策記錄。若 tokens 與此文件衝突，以 `global.css` 為準。

## Product Context

- **什麼是 vista.tw**：Vista Cheng（鄭緯筌）的個人官方網站與數位基地，累積 1,979+ 篇部落格文章、18,000+ 電子報訂閱者、200+ 場企業演講紀錄
- **目標讀者**：繁體中文圈的知識工作者、創作者、內容行銷從業者、企業內訓窗口；以及想用 AI 放大專業影響力的個人
- **產品類型**：個人品牌驅動的**編輯型部落格 + 名單磁鐵 + 工作坊導流**複合站點，主要流量目標是電子報訂閱與顧問洽詢
- **空間/產業**：內容創作 × AI 應用 × 個人品牌，對標的是 Medium、Substack、獨立寫作者官網（而非 SaaS 產品或企業行銷站）
- **多語系**：繁體中文（主）、英文、日文三語並存，各有獨立路由（`/`、`/en`、`/ja`）

## Aesthetic Direction

- **一句話方向**：**日系極簡 × 溫暖文青 × 雜誌質感**（`global.css` 首行註解）
- **情緒**：像一本厚實的紙本雜誌，不浮誇，不冷漠，有重量感，讀起來安靜但不疏離
- **裝飾層級**：**intentional**——以 typography 為主角，配上米色暖底與紅色強調色，不依賴圖形裝飾、漸層、3D、插畫
- **反模式（明確避開）**：
  - 深色主題（CLAUDE.md 明確禁止）
  - 紫色、霓虹、彩虹漸層
  - 圓潤卡通風（border-radius 保守）
  - AI slop 的三欄圖示網格、置中一切、泡泡按鈕
- **可類比的參考系**：端傳媒、天下雜誌官網、The Browser、Matter、Sidebar
- **設計起源**：Vista 長期以紙本雜誌閱讀為精神原鄉，把那種「翻頁的踏實感」帶進數位產品
